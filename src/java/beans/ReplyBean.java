...
public class ReplyBean{
    ...
    public List getReplys() {
        if(topicId!=null && !topicId.trim().equals("") && replys!=null && replys.isEmpty() && hasGetTpic) {
            //从数据库中查找该主题的所有回复
            List list = db.execSelect("select tbl_users.name, replyUser, content, submitDate, submitTime\n" +
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                reply.setAuthor((String)row.get(0));
                reply.setReplyUser((String)row.get(1));
                reply.setContent((String)row.get(2));
                reply.setSubmitDate((String)row.get(3));
                reply.setSubmitTime((String)row.get(4));
